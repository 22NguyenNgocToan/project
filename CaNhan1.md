                  ...
                  <Alert key={index} className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        <Badge variant={getSeverityColor(result.severity) as 'default' | 'secondary' | 'destructive' | 'outline'}>
...
const ReviewHistory: React.FC<ReviewHistoryProps> = ({ currentReview }) => {
  ...
  useEffect(() => {
    ...
    if (savedHistory) {
      const parsedHistory = JSON.parse(savedHistory).map((record: ReviewRecord & { timestamp: string }) => ({
        ...record,
